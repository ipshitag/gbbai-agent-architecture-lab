...
def initialize_default_agents() -> None:
    ...
    if (
        "MedicalResearchPlanner" in st.session_state.agents
        and "FinalMedicalReviewer" in st.session_state.agents
        and "MedicalResearcher" in st.session_state.agents
    ):
        agents_loaded = True
...
def display_agents() -> None:
    ...
    if "agents" in st.session_state:
        agent_tabs = st.sidebar.tabs(
            [
                f"{info['avatar']} {name}"
                for name, info in st.session_state.agents.items()
            ]
        )
        for tab, (agent_name, agent_info) in zip(
            agent_tabs, st.session_state.agents.items()
        ):
            agent = agent_info["object"]
            with tab:
                with st.form(key=f"form_{agent_name}"):
                    updated_system_message = st.text_area(
                        "📝 Task or Expertise",
                        value=agent_info["system_message"],
                        help="Describe the task or expertise of the agent. This helps in defining the agent's role and capabilities.",
                    )

                    # Get available deployments from session state
                    if (
                        "deployments" in st.session_state
                        and st.session_state.deployments
                    ):
                        available_deployments = list(
                            st.session_state.deployments.keys()
                        )
                        selected_deployment_name = st.radio(
                            "🧠 Choose your agent brain",
                            options=available_deployments,
                            index=available_deployments.index(agent_info["llm_config"])
                            if agent_info["llm_config"] in available_deployments
                            else 0,
                        ...
                        deployment_key = st.session_state.deployments[
                            selected_deployment_name
                        ]["key"]
                        deployment_endpoint = st.session_state.deployments[
                            selected_deployment_name
                        ]["endpoint"]
                        deployment_version = st.session_state.deployments[
                            selected_deployment_name
                        ]["version"]
                        ...
                        st.error(
                            "No deployments available. Please add a deployment in the Deployment Center."
                        )
